...
def get_contour_internal(r_c, r_t, area_ratio, L_c, theta_conv, theta_div, nozzle, R_1f, R_2f, R_3f, length_fraction, export_tikz):
    ...
    for i, (x_val, y_val) in enumerate(zip(xs, ys)):
        if i == 0:
            ...
            if x_val == processed_xs[-1]:
                ...
                contour = psf.regen.Contour(xs, ys)
                psf.regen.plot.plot_contour([contour])
                plt.show()
    ...
    if nozzle == "rao" and export_tikz:
        TikZ_rows = [
                 ["line", 0, L_c], 
                 ["arc", np.degrees(np.pi/2), np.degrees(np.pi/2) - np.degrees(theta_conv), R2], 
                 ["line", -np.degrees(theta_conv), -(x_cr - x_tl)/np.cos(theta_conv)], 
                 ["arc", -np.degrees(np.pi/2) - np.degrees(theta_conv), -np.degrees(np.pi/2), R1], 
                 ["arc", -np.degrees(np.pi/2), np.degrees(theta_n - np.pi/2), R3], 
                 ["arc", np.degrees(theta_n + np.pi/2), np.degrees(theta_e + np.pi/2), R1*20]]
        
        df = pd.DataFrame(TikZ_rows, columns=["type", "one", "two", "three"])
        df.to_csv(r"C:\Users\askho\MyFiles\Area\flamme_aerospace\paper\TikZ\thrust_chamber_contour.csv", index=False)
