...
internal_cross_section = psf.regen.CrossSectionRoundedInternal()
...
full_pass = psf.regen.CoolingCircuit(contour=RL10_contour, 
                                     ...
                                     span = [1.0, -1.0], 
                                     placement=psf.regen.SurfacePlacement(n_channel_positions=128), 
                                     channel_height=channel_height_fn)
def channel_width_fn(x):
    return 4e-3

def internal_channel_heihgt_fn(x):
    return 3e-3
internal_pass = psf.regen.CoolingCircuit(contour=RL10_contour, 
                                     coolant_transport=LH2_transport, 
                                     cross_section=internal_cross_section, 
                                     span = [-1.0, -0.3], 
                                     placement=psf.regen.InternalPlacement(n_channel_positions=8, n_channels_per_leaf=10, channel_width=channel_width_fn), 
                                     channel_height=internal_channel_heihgt_fn)


cooling_circuit_group = psf.regen.CoolingCircuitGroup(circuit_list=[full_pass, internal_pass])
...
def plot_comparison(x_list, y_list, labels,
    ...
    for (model, ds), m in zip(paper_datasets.items(), ref_markers):
        ax.plot(
            ...
            label=f'Paper: {model.replace("_", " ").title()}'
        )
...
labels = ['Sim: Static T', 'Sim: Static T']
plot_comparison(
...
plot_comparison(
    [cooling_data['x']], [q], ['Sim: Heat Flux'],
