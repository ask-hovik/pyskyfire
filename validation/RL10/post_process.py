...
3. Run `uv run --group docs python validation/RL10/post_process.py`. This loads
   both PKL files, regenerates the combined report in `validation/RL10/`, and
   rewrites the single-figure HTML files in `validation/RL10/standalone/`.
...
The `standalone/` directory holds `engine-3d.html`, `network.html`,
`wall-temperature.html`, `coolant-temperature.html`, `coolant-pressure.html`,
`heat-flux.html`, `temperature-profile-throat.html`,
`combustion-viscosity.html`, and the fuel-side `station-fuel-pressure.html`,
`station-fuel-temperature.html`, and `station-fuel-mass-flow.html`.
...
def save_standalone(figure, filename):
    """Write one report figure to ``standalone/`` as its own HTML file.

    The documentation embeds these single-figure files directly, so a plot
    shown on the capabilities page and the same plot in the report are always
    the same figure object.
    """
    os.makedirs(standalone_dir, exist_ok=True)
    path = os.path.join(standalone_dir, filename)
    figure.save_html(path)
    return figure
...
def add_regen_tabs(report, results):
    ...
    os.makedirs(standalone_dir, exist_ok=True)
    engine_viewer.save_html(os.path.join(standalone_dir, "engine-3d.html"))
    ...
    save_standalone(fig_wall_temp_comparison, "wall-temperature.html")
    ...
    save_standalone(fig_coolant_temperature, "coolant-temperature.html")
    ...
    save_standalone(fig_coolant_pressure, "coolant-pressure.html")
    tab2.add_figure(
        fig_coolant_pressure,
        caption=(
    ...
    save_standalone(fig_heat_flux, "heat-flux.html")
    ...
    save_standalone(throat_gradient, "temperature-profile-throat.html")
    ...
    for prop in ("M", "gamma", "T", "p", "h", "cp", "k", "mu", "Pr", "rho", "a", "v"):
        figure = psf.viz.PlotTransportProperty(
            thrust_chamber.combustion_transport,
            prop=prop,
            results=combustion_results,
        )
        if prop == "mu":
            save_standalone(figure, "combustion-viscosity.html")
        tab6.add_figure(figure)
...
def add_station_comparison_figures(
    tab, cycle_stations, reference_stations, side, standalone_slug=None
):
    """Add the pressure, temperature, and mass-flow comparisons for one side.

    ``standalone_slug`` names the ``standalone/station-<slug>-*.html`` exports;
    pass ``None`` for a side that is only shown inside the report.
    """
    ...
    for property_name, ylabel in STATION_PROPERTIES:
        ...
        if standalone_slug is not None:
            save_standalone(
                figure,
                f"station-{standalone_slug}-{standalone_names[property_name]}.html",
            )
        caption = None
...
def add_cycle_tabs(report, sizer_results, regen_results=None):
    ...
    viewer.save_html(path=os.path.join(standalone_dir, "network.html"))
