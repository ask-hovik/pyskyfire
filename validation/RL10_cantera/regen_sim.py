...
A_t = aerothermodynamics.A_t

cantera_combustion = psf.skycea.CombustionReaction.from_F_eps_Lstar(fu=params["cea_fu"], 
                                                                    ox=params["cea_ox"], 
                                                                    T_fu_in=params["T_gas_fu_in"], 
                                                                    T_ox_in=params["T_gas_ox_in"], 
                                                                    MR=params["MR"], 
                                                                    ...
                                                                    eps=params["eps"], 
                                                                    L_star=params["L_star"],
                                                                    mech=params["combustion_mechanism"],
                                                                    p_amb=1e5, # ca atmospheric pressure, optional
                                                                    npts=15 # good compromise between speed and precision
                                                                    ) 
...
print(f"CEA throat pressure: {CEA_optimals["p_t"]}")
print(f"Cantera throat pressure: {Cantera_optimals["p_t"]}")
# Generate report
report = psf.viz.Report("RL10 comparison")

# --------------  Engine parameters -----------------
tab_params = report.add_tab("Parameters")
tab_params.add_table(Cantera_optimals, caption="Cantera Optimum", key_title="Parameter", value_title="Value", precision = 3)
tab_params.add_table(CEA_optimals, caption="CEA Optimum", key_title="Parameters", value_title="Value", precision = 3)

# Save report to the same directory as the script
script_dir = os.path.dirname(os.path.abspath(__file__)) 
out_path = os.path.join(script_dir, "comparison_report.html")
